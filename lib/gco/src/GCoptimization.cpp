...
GCoptimization::GCoptimization(SiteID nSites, LabelID nLabels) :
		m_num_labels(nLabels), m_num_sites(nSites), m_datacostIndividual(0), m_smoothcostIndividual(
				0), m_labelcostsAll(0), m_labelcostsByLabel(0), m_labelcostCount(
				0), m_smoothcostFn(0), m_datacostFn(0), m_numNeighborsTotal(0), m_queryActiveSitesExpansion(
				&GCoptimization::queryActiveSitesExpansion<DataCostFnFromArray>), m_setupDataCostsSwap(
				0), m_setupDataCostsExpansion(0), m_setupSmoothCostsSwap(0), m_setupSmoothCostsExpansion(
				0), m_applyNewLabeling(0), m_updateLabelingDataCosts(0), m_giveSmoothEnergyInternal(
				0), m_solveSpecialCases(
				&GCoptimization::solveSpecialCases<DataCostFnFromArray>), m_datacostFnDelete(
				0), m_smoothcostFnDelete(0), m_random_label_order(false), m_verbosity(
				0), m_labelingInfoDirty(true), m_lookupSiteVar(
				new SiteID[nSites]), m_labeling(new LabelID[nSites]), m_labelTable(
				new LabelID[nLabels]), m_labelingDataCosts(
				new EnergyTermType[nSites]), m_labelCounts(new SiteID[nLabels]), m_activeLabelCounts(
				new SiteID[m_num_labels]), m_stepsThisCycle(0), m_stepsThisCycleTotal(
				0) {
	if (nLabels <= 1)
...
template<>
GCoptimization::SiteID GCoptimization::queryActiveSitesExpansion<
		GCoptimization::DataCostFnSparse>(LabelID alpha_label,
		SiteID *activeSites) {
	return ((DataCostFnSparse*) m_datacostFn)->queryActiveSitesExpansion(
			alpha_label, m_labeling, activeSites);
}
...
void GCoptimization::specializeDataCostFunctor(const UserFunctor f) {
	...
	m_queryActiveSitesExpansion = &GCoptimization::queryActiveSitesExpansion<
			UserFunctor>;
	m_setupDataCostsExpansion = &GCoptimization::setupDataCostsExpansion<
			UserFunctor>;
	...
	m_updateLabelingDataCosts = &GCoptimization::updateLabelingDataCosts<
			UserFunctor>;
...
void GCoptimization::specializeSmoothCostFunctor(const UserFunctor f) {
	...
	m_giveSmoothEnergyInternal = &GCoptimization::giveSmoothEnergyInternal<
			UserFunctor>;
	m_setupSmoothCostsExpansion = &GCoptimization::setupSmoothCostsExpansion<
			UserFunctor>;
...
GCoptimization::EnergyType GCoptimization::giveSmoothEnergyInternal() {
	...
	for (i = 0; i < m_num_sites; i++) {
		...
		for (n = 0; n < numN; n++) {
			...
			if (nSite < i)
				eng += weights[n]
						* (sc->compute(i, nSite, m_labeling[i],
								m_labeling[nSite]));
		}
	}
...
OLGA_INLINE void GCoptimization::addterm1_checked(EnergyT* e, VarID i,
		EnergyTermType e0, EnergyTermType e1) {
	if (e0 > GCO_MAX_ENERGYTERM || e1 > GCO_MAX_ENERGYTERM)
		handleError(
				"Data cost term was larger than GCO_MAX_ENERGYTERM; danger of integer overflow.");
	m_beforeExpansionEnergy += e1;
	e->add_term1(i, e0, e1);
}

OLGA_INLINE void GCoptimization::addterm1_checked(EnergyT* e, VarID i,
		EnergyTermType e0, EnergyTermType e1, EnergyTermType w) {
	if (e0 > GCO_MAX_ENERGYTERM || e1 > GCO_MAX_ENERGYTERM)
		handleError(
				"Smooth cost term was larger than GCO_MAX_ENERGYTERM; danger of integer overflow.");
	if (w > GCO_MAX_ENERGYTERM)
		handleError(
				"Smoothness weight was larger than GCO_MAX_ENERGYTERM; danger of integer overflow.");
...
OLGA_INLINE void GCoptimization::addterm2_checked(EnergyT* e, VarID i, VarID j,
		EnergyTermType e00, EnergyTermType e01, EnergyTermType e10,
		...
		for (n = 0; n < nNum; n++) {
			...
			if (m_lookupSiteVar[nSite] == -1)
				addterm1_checked(e, i,
						sc->compute(site, nSite, alpha_label,
								m_labeling[nSite]),
						sc->compute(site, nSite, m_labeling[site],
								m_labeling[nSite]), weights[n]);
			else if (nSite < site) {
...
void GCoptimization::setupDataCostsSwap(SiteID size, LabelID alpha_label,
		LabelID beta_label, EnergyT *e, SiteID *activeSites) {
	DataCostT* dc = (DataCostT*) m_datacostFn;
	for (SiteID i = 0; i < size; i++) {
...
bool GCoptimization::solveSpecialCases(EnergyType& energy) {
	...
	if (dc && !sc && !lc) {
		...
		for (SiteID i = 0; i < m_num_sites; ++i) {
			...
			if (minCostLabel > GCO_MAX_ENERGYTERM)
				handleError(
						"Data cost was larger than GCO_MAX_ENERGYTERM; danger of integer overflow.");
	...
	if (!dc && !sc && lc) {
		...
		LabelID minLabel = 0;
		EnergyType minLabelCost = GCO_MAX_ENERGYTERM
				* (EnergyType) m_num_labels;
	...
	try {
		...
		for (LabelID l = 0; l < m_num_labels; ++l) {
			...
			e[l] += (EnergyType) (m_num_sites - iter.feasibleSites())
					* GCO_MAX_ENERGYTERM; // pre-add GCO_MAX_ENERGYTERM for all infeasible sites
			for (; !iter.done(); ++iter) {
		...
		for (LabelID alpha_count = 1; alpha_count <= m_num_labels;
				++alpha_count) {
			checkInterrupt();
			ticks0 = gcoclock();
...
void GCoptimization::setDataCost(LabelID l, SparseDataCost *costs,
	...
	if (!m_datacostFn)
		specializeDataCostFunctor(DataCostFnSparse(numSites(), numLabels()));
			...
			!= (SiteID (GCoptimization::*)(LabelID,
					SiteID*))&GCoptimization::queryActiveSitesExpansion<DataCostFnSparse>)handleError("Cannot apply sparse data costs after dense data costs have been used.");
	m_labelingInfoDirty = true;
	DataCostFnSparse* dc = (DataCostFnSparse*)m_datacostFn;
...
void GCoptimization::setSmoothCost(EnergyTermType *smoothArray) {
	specializeSmoothCostFunctor(
			SmoothCostFnFromArray(smoothArray, m_num_labels));
}
...
void GCoptimization::setSmoothCostFunctor(SmoothCostFunctor* f) {
	...
	m_setupSmoothCostsExpansion = &GCoptimization::setupSmoothCostsExpansion<
			SmoothCostFunctor>;
	m_setupSmoothCostsSwap = &GCoptimization::setupSmoothCostsSwap<
			SmoothCostFunctor>;
}
...
void GCoptimization::setLabelSubsetCost(LabelID* labels, LabelID numLabels,
	...
	if (cost > GCO_MAX_ENERGYTERM)
		handleError(
				"Label cost was larger than GCO_MAX_ENERGYTERM; danger of integer overflow.");
	...
	for (LabelCostIter* lci = m_labelcostsByLabel[labels[0]]; lci;
		...
		if (numLabels == lci->node->numLabels) {
			if (!memcmp(labels, lci->node->labels,
					numLabels * sizeof(LabelID))) {
...
//--------------------------------------------------------------------
float GCoptimization::dynamic_programming(
		vector<vector<IcgBench::Seed>> &labelSuperPixels, int numSuperPixels, CImg<float> *dataTermCuda) {
	float new_energy, old_energy;

	printStatus1("starting dynamic programming without improvements");
	//new_energy = compute_energy();

	//Call recursive
	gcoclock_t ticks0 = gcoclock();

	int tam = numSuperPixels * m_num_labels;
	long long t[tam];

	int l[numSuperPixels];
	...
	new_energy = dynamic_programmingRec(numSuperPixels - 1, 0, labelSuperPixels,
			t, l, dataTermCuda);

	for (int i = 0; i < numSuperPixels; ++i) {
			vector<IcgBench::Seed> super = labelSuperPixels[i];
			for (unsigned int j = 0; j < super.size(); ++j) {
					m_labeling[super[j].label] = (m_num_labels - 1) - l[i];
			}
	}
...
float GCoptimization::dynamic_programmingRec(
		int superPixel, long level,
		vector<vector<IcgBench::Seed>> &labelSuperPixels, long long *t,
		int *l, CImg<float> *dataTermCuda) {

	float new_energy, min_energy, energy;
	LabelID min;
	...
	if (superPixel == -1) {
		...
		int index = (superPixel) * m_num_labels + level;
		if (t[index] != -1) {
			return t[index];
		}

		min = GCO_MAX_ENERGYTERM;

		//min_energy = GCO_MAX_ENERGYTERM;
		...
		float dataCost;
		//float minDataCost = -1;
		vector<IcgBench::Seed> super = labelSuperPixels[superPixel];

		for (int i = 0; i < m_num_labels; i++) {

			dataCost = 0;
			for (int j = 0; j < super.size(); ++j) {
				//setLabel(super[j].x, i);
				//m_labeling[super[j].label] = i;
				dataCost += (*dataTermCuda)(super[j].x, super[j].y, 0, i);
			...
			energy =  dataCost + dynamic_programmingRec(superPixel - 1, level + 1,
							labelSuperPixels, t, l, dataTermCuda);
			//if ()
			//energy = energy > penergy ? penergy : energy;
				...
				do {
					if (!alpha_expansion(m_labelTable[next]))
						std::swap(m_labelTable[next],
								m_labelTable[--queueSize]); // don't put this label in a new queue
					else
...
GCoptimization::EnergyType GCoptimization::setupLabelCostsExpansion(SiteID size,
		LabelID alpha_label, EnergyT *e, SiteID *activeSites) {
	...
	if (m_queryActiveSitesExpansion
			== (SiteID (GCoptimization::*)(LabelID,
					SiteID*))&GCoptimization::queryActiveSitesExpansion<DataCostFnSparse>) {
...
memset	(m_activeLabelCounts,0,m_num_labels*sizeof(SiteID));
	...
	if (!m_labelCounts[alpha_label]) {
		for (LabelCostIter* lci = m_labelcostsByLabel[alpha_label]; lci; lci =
				lci->next)
			if (!lci->node->active)
...
void GCoptimization::updateLabelingInfo(bool updateCounts, bool updateActive,
	...
	if (m_labelcostsAll) {
		...
		if (updateActive) {
			...
			for (LabelID l = 0; l < m_num_labels; ++l)
				if (m_labelCounts[l])
					for (LabelCostIter* lci = m_labelcostsByLabel[l]; lci; lci =
							lci->next)
						lci->node->active = true;
		}
	}
	...
	if (updateCosts) {
		if (m_updateLabelingDataCosts)
			...
			memset(m_labelingDataCosts, 0,
					m_num_sites * sizeof(EnergyTermType));
	}
}
...
bool GCoptimization::alpha_expansion(LabelID alpha_label) {
	...
	try {
		...
		if (m_queryActiveSitesExpansion)
			size = (this->*m_queryActiveSitesExpansion)(alpha_label,
					activeSites);
		...
		if (m_setupDataCostsExpansion)
			(this->*m_setupDataCostsExpansion)(size, alpha_label, &e,
					activeSites);
		if (m_setupSmoothCostsExpansion)
			(this->*m_setupSmoothCostsExpansion)(size, alpha_label, &e,
					activeSites);
		EnergyType alphaCorrection = setupLabelCostsExpansion(size, alpha_label,
				&e, activeSites);
		checkInterrupt();
...
void GCoptimization::alpha_beta_swap(LabelID alpha_label, LabelID beta_label) {
	assert(
			alpha_label >= 0 && alpha_label < m_num_labels && beta_label >= 0
	...
	try {
		...
		for (SiteID i = 0; i < size; i++) {
			m_labeling[activeSites[i]] =
					(e.get_var(i) == 0) ? alpha_label : beta_label;
...
void GCoptimizationGridGraph::setupNeighbData(SiteID startY, SiteID endY,
		SiteID startX, SiteID endX, SiteID maxInd, SiteID *indexes) {
	SiteID x, y, pix;
	SiteID n;
...
void GCoptimizationGridGraph::setSmoothCostVH(EnergyTermType *smoothArray,
		EnergyTermType *vCosts, EnergyTermType *hCosts) {
...
void GCoptimizationGeneralGraph::setNeighbors(SiteID site1, SiteID site2,
	...
	assert(
			site1 < m_num_sites && site1 >= 0 && site2 < m_num_sites
					&& site2 >= 0);
...
void GCoptimizationGeneralGraph::setAllNeighbors(SiteID *numNeighbors,
		SiteID **neighborsIndexes, EnergyTermType **neighborsWeights) {
...
GCoptimization::DataCostFnSparse::DataCostFnSparse(SiteID num_sites,
		LabelID num_labels) :
		m_num_sites(num_sites), m_num_labels(num_labels), m_buckets_per_label(
				(m_num_sites + cSitesPerBucket - 1) / cSitesPerBucket), m_buckets(
				0) {
}

GCoptimization::DataCostFnSparse::DataCostFnSparse(const DataCostFnSparse& src) :
		m_num_sites(src.m_num_sites), m_num_labels(src.m_num_labels), m_buckets_per_label(
				src.m_buckets_per_label), m_buckets(0) {
...
void GCoptimization::DataCostFnSparse::set(LabelID l,
		const SparseDataCost* costs, SiteID count) {
	...
	for (int i = 0; i < m_buckets_per_label; ++i) {
		...
		while (next < end && next->site < end_site) {
			if (next->site < 0 || next->site >= m_num_sites)
				throw GCException(
						"Invalid site id given for sparse data cost; must be within range.");
...
GCoptimization::EnergyTermType GCoptimization::DataCostFnSparse::search(
		DataCostBucket& b, SiteID s) {
	...
	do {
		const SparseDataCost* mid = (const SparseDataCost*) ((((size_t) L
				+ (size_t) R) >> 1) & cDataCostPtrMask);
		if (s < mid->site)
...
OLGA_INLINE GCoptimization::EnergyTermType GCoptimization::DataCostFnSparse::compute(
		SiteID s, LabelID l) {
	DataCostBucket& b = m_buckets[l * m_buckets_per_label
			+ (s >> cLogSitesPerBucket)];
	if (b.begin == b.end)
	...
	if (b.predict < b.end) {
		...
		if (b.predict->site > s && b.predict > b.begin
				&& (b.predict - 1)->site < s)
			return GCO_MAX_ENERGYTERM;
	}
...
GCoptimization::SiteID GCoptimization::DataCostFnSparse::queryActiveSitesExpansion(
	...
	const SparseDataCost* end = m_buckets[alpha_label * m_buckets_per_label
			+ m_buckets_per_label - 1].end;
	SiteID count = 0;
	for (; next < end; ++next) {
